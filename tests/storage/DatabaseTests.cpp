...
TEST_F(DatabaseTest, ThrowsWhenSqlExecutionFails)
{
	...
	EXPECT_THROW(
		database.Execute("CREATE TABLE invalid SQL;"),
		std::runtime_error
	);
}
...
TEST_F(DatabaseTest, EnablesForeignKeyConstraints)
{
	...
	EXPECT_THROW(
		database.Execute(
			"INSERT INTO child_records (parent_id) VALUES (42);"
		),
		std::runtime_error
	);
}
...
TEST_F(DatabaseTest, TransfersConnectionOwnershipWhenMoved)
{
	...
	EXPECT_NO_THROW(
		moved_database.Execute(
			"CREATE TABLE moved_connection (id INTEGER PRIMARY KEY);"
		)
	);
}
...
TEST_F(DatabaseTest, ThrowsWhenDatabaseCannotBeOpened)
{
	...
	const std::filesystem::path invalid_path =
		missing_directory / "database.sqlite3";

	EXPECT_THROW(
		Storage::Database database(invalid_path),
		std::runtime_error
	);
}
