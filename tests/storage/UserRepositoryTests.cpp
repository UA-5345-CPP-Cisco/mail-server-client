...
{
	const std::int64_t user_id = m_repository->CreateUser(
		"alice",
		"alice@example.com",
		"password-hash"
	);

	const std::optional<Storage::UserRecord> user_by_id =
		m_repository->FindById(user_id);
	const std::optional<Storage::UserRecord> user_by_email =
		m_repository->FindByEmail("alice@example.com");
	const std::optional<Storage::UserRecord> user_by_username =
		m_repository->FindByUsername("alice");
...
TEST_F(UserRepositoryTest, ReturnsEmptyOptionalWhenUserDoesNotExist)
{
	...
	EXPECT_FALSE(
		m_repository->FindByEmail("missing@example.com").has_value()
	);
...
TEST_F(UserRepositoryTest, UpdatesUserStatus)
{
	...
	EXPECT_TRUE(
		m_repository->UpdateStatus(
			user_id,
			Storage::UserStatus::Disabled
		)
	);
	EXPECT_FALSE(
		m_repository->UpdateStatus(
			user_id + 1,
			Storage::UserStatus::Active
		)
	);
...
TEST_F(UserRepositoryTest, TreatsInputAsBoundValues)
{
	...
	const std::optional<Storage::UserRecord> user =
		m_repository->FindByUsername(username);
