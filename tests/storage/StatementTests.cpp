...
{
	Storage::Statement insert_statement(
		*m_database,
		R"SQL(
	...
	Storage::Statement select_statement(
		*m_database,
		R"SQL(
			SELECT
				id,
	...
	EXPECT_EQ(
		select_statement.ColumnText(2),
		"value'); DROP TABLE statement_values; --"
	);
...
{
	EXPECT_THROW(
		Storage::Statement statement(*m_database, "INVALID SQL"),
		std::runtime_error
	);
}
