...
namespace
{
	...
	protected:
		...
		{
			handler.HandleEvent(
				{smtp::SmtpEventType::MessageReceived, connectionId_, line},
				state,
				context);
		}
	...
	TEST_F(SessionStorageTest, StoresAcceptedMessageAndQueuesRecipients)
	{
		...
		Storage::MigrationRunner migrations(
			database,
			SMTP_SERVER_TEST_MIGRATIONS_DIR);
		migrations.Run();
		...
		EXPECT_EQ(
			recipients.front().delivery_status,
			Storage::DeliveryStatus::Queued);
		EXPECT_EQ(
			recipients.back().delivery_status,
			Storage::DeliveryStatus::Queued);
	}
