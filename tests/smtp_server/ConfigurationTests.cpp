...
TEST_F(ConfigurationTest, LoadsAllConfigurationSections)
{
  ...
  EXPECT_EQ(configuration.Server().delivery.pollIntervalMilliseconds, 250);
...
TEST_F(ConfigurationTest, UsesDefaultsForMissingValues)
{
  ...
  EXPECT_EQ(configuration.Database().migrationsPath, "libs/storage/migrations");
  ...
  EXPECT_EQ(configuration.Server().delivery.pollIntervalMilliseconds, 100);
