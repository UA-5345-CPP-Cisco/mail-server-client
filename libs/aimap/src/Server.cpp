  ...
  try
  {
    ...
    if (!request.contains("action"))
    {
      SendResponse(client_fd,
                   R"({"status": "ERROR", "message": "Missing 'action' field"})" +
                     std::string("\n"));
      return;
    }
...
void Server::SendResponse(int client_fd, const std::string& raw_json)
{
  ...
  {
    ssize_t bytes =
      send(client_fd, raw_json.c_str() + total_bytes, raw_json.size() - total_bytes, MSG_NOSIGNAL);
    if (bytes == -1)
    {
      if (errno == EAGAIN || errno == EWOULDBLOCK)
      {
...
void Server::HandleLogin(int client_fd, const std::string& email, const std::string& password)
{
  ...
  std::cout << "[Thread " << std::this_thread::get_id() << "] Login attempt from: " << email
            << "\n";
  SendResponse(client_fd,
               R"({"status": "OK", "message": "Logged in successfully"})" + std::string("\n"));
}

void Server::HandleListEmails(int client_fd)
{
  SendResponse(client_fd,
               R"({"status": "OK", "emails": [{"id": 1, "subject": "Welcome!"}]})" +
                 std::string("\n"));
}
...
{
  SendResponse(client_fd,
               R"({"status": "OK", "body": "This is a test email body."})" + std::string("\n"));
}
