...
std::string Logger::GetCurrentTime()
{
  ...
  oss << std::put_time(&tm_info, "%d/%m/%Y %H:%M:%S") << ":" << std::setfill('0') << std::setw(3)
      << ms.count();
  return oss.str();
}
...
void Logger::LogProd(const std::string_view& func, const std::string_view& msg)
{
  ...
  this->logger_fp << std::this_thread::get_id() << " - " << GetCurrentTime() << " [1] - [" << func
                  << "] " << msg << "\n";
  if (this->flush)
  {
...
void Logger::LogDebug(const std::string_view& func, const std::string_view& msg)
{
  ...
  std::lock_guard<std::mutex> lock(this->logger_mutex);
  this->logger_fp << std::this_thread::get_id() << " - " << GetCurrentTime() << " [2] - [" << func
                  << "] " << msg << "\n";
  if (this->flush)
  {
    this->logger_fp.flush();
...
void Logger::LogTrace(const std::string_view& func,
                      const std::string_view& input,
                      const std::string_view& ret_val)
{
  ...
  std::lock_guard<std::mutex> lock(this->logger_mutex);
  this->logger_fp << std::this_thread::get_id() << " - " << GetCurrentTime() << " [3] - [" << func
                  << "] Input: " << input << " | Return: " << ret_val << "\n";
  if (this->flush)
  {
    this->logger_fp.flush();
  }
