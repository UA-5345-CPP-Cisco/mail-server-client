...
UserCache::UserCache(Storage::CacheStore& store) : m_store(store)
{
}

void UserCache::Save(const CachedUserProfile& profile, std::chrono::seconds ttl)
{
  m_store.Put(Namespace().toStdString(),
              Key().toStdString(),
...
{
  const auto entry = m_store.Get(Namespace().toStdString(), Key().toStdString(), allow_stale);
