...
AuthHandler::RegisterUser(const QString& username, const QString& email, const QString& password)
{
    ISXService::Service::Logger().Log(Logging::LogLevel::Info,
                                      "Registration attempt started for email: " + email.toStdString());
    try {
        ...
        if (!response.is_success()) {
            ...
            ISXService::Service::Logger().Log(Logging::LogLevel::Error,
                                              "Registration failed with server error code: " +
        ...
        ISXService::Service::Logger().Log(Logging::LogLevel::Info,
                                          "Registration successful for email: " + email.toStdString());
...
AuthHandler::AuthResult AuthHandler::LoginUser(const QString& email, const QString& password)
{
    ...
    try {
        ...
        if (!response.is_success()) {
            ISXService::Service::Logger().Log(Logging::LogLevel::Error,
                                              "Login failed with server error code: " +
                                                  std::to_string(response.status_code));
        ...
        if (!response.body.is_object()) {
            ISXService::Service::Logger().Log(Logging::LogLevel::Error,
                                              "Login failed: Response body is not a JSON object");
            return AuthResult::InternalError;
        }
        ...
        if (user_value == nullptr || !user_value->is_object()) {
            ISXService::Service::Logger().Log(Logging::LogLevel::Error,
                                              "Login failed: Missing or invalid 'user' object in JSON");
            return AuthResult::InternalError;
        }
        ...
        if (username_value == nullptr || email_value == nullptr || !username_value->is_string() ||
            ...
            ISXService::Service::Logger().Log(Logging::LogLevel::Error,
                                              "Login failed: Missing username or email in response payload");
            return AuthResult::InternalError;
        }
        ...
        ISXService::Service::Logger().Log(Logging::LogLevel::Info,
                                          "Login successful for email: " + email.toStdString());
