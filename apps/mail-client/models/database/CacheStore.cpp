...
namespace Storage {
    ...
    void CacheStore::EnsureSchema() const
    {
        ISXService::Service::Logger().Log(Logging::LogLevel::Info,
                                          "CacheStore::EnsureSchema: executing schema creation");
        m_database.Execute(kCacheSchema);
    }

    void CacheStore::Put(const std::string& cache_namespace,
                         ...
                         std::int64_t version)
    {
        ...
        const auto expires_at = std::chrono::system_clock::now() + ttl;
        {
            ...
            oss << "CacheStore::Put: namespace=" << cache_namespace << " key=" << cache_key << " version=" << version
                << " ttl=" << ttl.count();
        ...
        ISXService::Service::Logger().Log(
            ...
            (std::string("CacheStore::Put: stored entry for namespace=") + cache_namespace + " key=" + cache_key));
    }

    std::optional<CacheStore::Entry>
    CacheStore::Get(const std::string& cache_namespace, const std::string& cache_key, bool allow_stale) const
    {
        Statement statement(m_database,
                            R"SQL(
			SELECT
        ...
        if (!statement.Step()) {
            ISXService::Service::Logger().Log(
                Logging::LogLevel::Debug,
                (std::string("CacheStore::Get: miss for namespace=") + cache_namespace + " key=" + cache_key));
            return std::nullopt;
        }
        ...
        if (!allow_stale && entry.expires_at <= std::chrono::system_clock::now()) {
            ISXService::Service::Logger().Log(Logging::LogLevel::Debug,
                                              (std::string("CacheStore::Get: entry expired namespace=") +
                                               entry.cache_namespace + " key=" + entry.cache_key));
            return std::nullopt;
        }

        ISXService::Service::Logger().Log(Logging::LogLevel::Debug,
                                          (std::string("CacheStore::Get: hit namespace=") + entry.cache_namespace +
                                           " key=" + entry.cache_key + " version=" + std::to_string(entry.version)));
        return entry;
    }

    bool CacheStore::Exists(const std::string& cache_namespace, const std::string& cache_key) const
    {
        return Get(cache_namespace, cache_key, true).has_value();
    }

    void CacheStore::Invalidate(const std::string& cache_namespace, const std::string& cache_key)
    {
        Statement statement(m_database,
                            R"SQL(
        ...
        ISXService::Service::Logger().Log(
            Logging::LogLevel::Debug,
            (std::string("CacheStore::Invalidate: deleting namespace=") + cache_namespace + " key=" + cache_key));
        statement.Step();
    }
    ...
    void CacheStore::InvalidateNamespace(const std::string& cache_namespace)
    {
        ...
        ISXService::Service::Logger().Log(
            Logging::LogLevel::Debug,
            (std::string("CacheStore::InvalidateNamespace: deleting namespace=") + cache_namespace));
        statement.Step();
    }

    void CacheStore::InvalidateNamespacePrefix(const std::string& cache_namespace_prefix)
    {
        Statement statement(m_database,
                            R"SQL(
			DELETE FROM cache_entries
        ...
        ISXService::Service::Logger().Log(
            Logging::LogLevel::Debug,
            (std::string("CacheStore::InvalidateNamespacePrefix: prefix=") + cache_namespace_prefix));
        statement.Step();
    }
    ...
    std::string CacheStore::TimePointToString(const std::chrono::system_clock::time_point& time_point) const
    {
    ...
    std::chrono::system_clock::time_point CacheStore::StringToTimePoint(const std::string& text) const
    {
        std::tm tm{};
        ...
        if (stream.fail()) {
            ISXService::Service::Logger().Log(
                ...
                (std::string("CacheStore::StringToTimePoint: Failed to parse cache timestamp: ") + text));
        ...
        if (time_value == -1) {
            ISXService::Service::Logger().Log(
                Logging::LogLevel::Error,
                "CacheStore::StringToTimePoint: Failed to convert broken-down time to UTC time_t");
