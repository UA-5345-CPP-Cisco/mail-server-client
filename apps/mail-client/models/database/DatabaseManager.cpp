...
namespace ISXDatabaseManager {
    ...
    void DatabaseManager::EnsureInitialized()
    {
        ISXService::Service::Logger().Log(Logging::LogLevel::Info,
                                          std::string("DatabaseManager::EnsureInitialized: database_path=") +
                                              DatabasePath().string());
        Storage::Database database(DatabasePath());
        ISXService::Service::Logger().Log(Logging::LogLevel::Info,
                                          std::string("DatabaseManager::EnsureInitialized: migrations_path=") +
                                              MigrationsPath().string());
        Storage::MigrationRunner runner(database, MigrationsPath());
        runner.Run();
        ISXService::Service::Logger().Log(Logging::LogLevel::Info,
                                          "DatabaseManager::EnsureInitialized: migrations applied");
    }
    ...
    std::filesystem::path DatabaseManager::MigrationsPath()
    {
        ...
        if (std::filesystem::exists(current_path)) {
            ISXService::Service::Logger().Log(
                ...
                (std::string("DatabaseManager::MigrationsPath: using current_path=") + current_path.string()));
            return current_path;
        }
        ...
        for (int depth = 0; depth < 6; ++depth) {
            ...
            ISXService::Service::Logger().Log(
                Logging::LogLevel::Debug,
                (std::string("DatabaseManager::MigrationsPath: probing candidate=") + candidate.string()));
            if (std::filesystem::exists(candidate)) {
                ISXService::Service::Logger().Log(
                    Logging::LogLevel::Debug,
                    (std::string("DatabaseManager::MigrationsPath: found candidate=") + candidate.string()));
                return candidate;
            }
        ...
        ISXService::Service::Logger().Log(
            ...
            (std::string("DatabaseManager::MigrationsPath: falling back to current_path=") + current_path.string()));
        return current_path;
    }
    ...
    std::filesystem::path DatabaseManager::ResolveAppDataDirectory()
    {
        ...
        if (app_data_dir.isEmpty()) {
            ...
            ISXService::Service::Logger().Log(
                Logging::LogLevel::Warning,
                std::string("DatabaseManager::ResolveAppDataDirectory: writableLocation was empty, using ") +
                    app_data_dir.toStdString());
        } else {
            ISXService::Service::Logger().Log(Logging::LogLevel::Debug,
                                              std::string("DatabaseManager::ResolveAppDataDirectory: using ") +
                                                  app_data_dir.toStdString());
        }
        ...
        if (!dir.exists(app_data_dir)) {
            ...
            ISXService::Service::Logger().Log(
                ...
                std::string("DatabaseManager::ResolveAppDataDirectory: created directory ") +
                    app_data_dir.toStdString());
        }
