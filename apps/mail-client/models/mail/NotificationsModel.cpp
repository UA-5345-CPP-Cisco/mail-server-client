...
namespace ISXMail {
    ...
    void NotificationsModel::showNotification(const QString& title, const QString& body, int timeoutMs)
    {
        ISXService::Service::Logger().Log(Logging::LogLevel::Info,
                                          "showNotification: " + title.toStdString() + " - " + body.toStdString() +
    ...
    void NotificationsModel::quitAppAfterDelay(int delayMs)
    {
        ISXService::Service::Logger().Log(
            Logging::LogLevel::Info, "quitAppAfterDelay: scheduling app exit in " + std::to_string(delayMs) + " ms");
    ...
    void NotificationsModel::onNewInboxMessage(const QString& sender, const QString& subject, const QString& preview)
    {
        ...
        ISXService::Service::Logger().Log(Logging::LogLevel::Info,
                                          "onNewInboxMessage: scheduling notification in 10 seconds");
