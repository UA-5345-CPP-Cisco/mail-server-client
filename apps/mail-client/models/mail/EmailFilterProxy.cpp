...
bool EmailFilterProxy::filterAcceptsRow(int sourceRow, const QModelIndex& sourceParent) const
    {
      ...
      const bool is_archive = src->data(idx, ArchiveRole).toBool();

      switch (m_type) {
        case Inbox:   return is_inbox || (!is_sent && !is_draft) && !is_archive;
        case Sent:    return is_sent && !is_inbox && !is_archive;
        case Starred: return is_starred && !is_archive;
        case Draft:   return is_draft && !is_archive;
