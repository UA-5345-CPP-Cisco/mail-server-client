...
Response SetRead(Request const& request, bool read)
{
  ...
  try
  {
    ...
    const bool updated = ServiceRegistry::Storage().SetRead(message_id, user_email, read);
    if (!updated)
    {
      return MakeError(request, http::status::not_found, "Message not found for recipient");
    }

    ServiceRegistry::Logger().Log(LogLevel::Info,
                                  "Message " + std::to_string(message_id) + (read ? " read" : " unread"));

    return MakeJsonResponse(
      request, http::status::ok, json::object{{"id", message_id}, {"read", read}, {"status", "ok"}});
  }
...
{
  return SetRead(request, true);
}

Response UnreadHandler(Request const& request)
{
  return SetRead(request, false);
}
