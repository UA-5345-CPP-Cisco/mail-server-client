...
void SmtpServer::RouteEvent(SmtpEvent event)
{
  ...
  {
    dependencies_.logger.Log(Logging::LogLevel::Warning, "SMTP connection limit reached");
...
std::shared_ptr<SmtpSession> SmtpServer::GetOrCreateSession(ConnectionId connectionId)
{
  ...
  SmtpSessionContext context{config_,
  ...
  auto session = std::make_shared<SmtpSession>(connectionId, context, dependencies_.sessionHandler);
