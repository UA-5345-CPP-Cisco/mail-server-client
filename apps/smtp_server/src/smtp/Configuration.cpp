...
namespace smtp
{
	...
	void Configuration::Load(const std::filesystem::path &path)
	{
		...
		try
		{
			...
			ReadValue(
				tree,
				"server.max_message_size_bytes",
				server_.maxMessageSizeBytes);
			ReadValue(
				tree,
				"server.require_authentication",
				server_.requireAuthentication);
			ReadValue(
				tree,
			...
			ReadValue(
				tree,
				"delivery.batch_size",
				server_.delivery.batchSize);
			ReadValue(
				tree,
				"delivery.poll_interval_ms",
				server_.delivery.pollIntervalMilliseconds);
			ReadValue(tree, "tls.enabled", server_.tls.enabled);
			ReadValue(
				tree,
				"tls.certificate_path",
				server_.tls.certificatePath);
			ReadValue(
				tree,
				"tls.private_key_path",
				server_.tls.privateKeyPath);

			const unsigned int port =
				tree.get<unsigned int>("server.port", server_.port);
		...
		catch (const boost::property_tree::json_parser_error &error)
		{
			throw std::runtime_error(
				"Unable to read SMTP configuration '" + path.string() +
				"': " + error.message());
		}
		catch (const boost::property_tree::ptree_error &error)
		{
			throw std::runtime_error(
				"Invalid SMTP configuration '" + path.string() +
				"': " + error.what());
		}
	}
	...
	void Configuration::Validate() const
	{
		...
		{
			throw std::runtime_error(
				"server.max_message_size_bytes must be greater than zero");
		}
		if (server_.delivery.batchSize == 0)
		{
			throw std::runtime_error(
				"delivery.batch_size must be greater than zero");
		}
		if (server_.delivery.pollIntervalMilliseconds == 0)
		{
			throw std::runtime_error(
				"delivery.poll_interval_ms must be greater than zero");
		}
		if (server_.tls.enabled &&
			(server_.tls.certificatePath.empty() ||
			 server_.tls.privateKeyPath.empty()))
		{
			throw std::runtime_error(
				"tls.certificate_path and tls.private_key_path are required "
				"when TLS is enabled");
		}
	}
