...
struct BoostSocketsManager::Implementation
{
  ...
  void StartAccept()
  {
    ...
    acceptor->async_accept(stream->next_layer(),
                           [this, stream](const boost::system::error_code& error)
                           {
                             if (!isRunning)
                             {
                               return;
  ...
  {
    asio::post(ioContext,
               [this, connectionId, message = std::move(message)]() mutable
               {
                 auto connection = FindConnection(connectionId);
...
BoostSocketsManager::BoostSocketsManager() : implementation_(std::make_unique<Implementation>())
{
}
