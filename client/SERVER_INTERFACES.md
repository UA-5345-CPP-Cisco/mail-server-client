...
- The QML account login and registration screens emit signals, but the actual server authentication logic is not implemented yet.
...
Notes:
- This is the main interface used when the user clicks `Send` in [`screens/navigation/new_message/NewMessageQML.qml`](./screens/navigation/new_message/NewMessageQML.qml).
...
Notes:
- Used when a compose window is closed with unsent content in [`screens/navigation/new_message/NewMessageQML.qml`](./screens/navigation/new_message/NewMessageQML.qml).
...
- Routes to login/registration screens and contains the `Connections` object that currently closes the dialog without server work.
...
If the goal is true client-to-server documentation, the missing piece is a network transport layer or API client. At the moment, these interfaces only cover:
